...
int num[1001][1001],goy[4] = {0,0,1,-1},gox[4] = {1,-1,0,0},n,m,f[1001];
...
//	std::cout<<std::endl;
	...
	for(int i = 0;i<m;i++)
	{
		if(state[n-1][i]==true&&l==-1)l = i;
		if(state[n-1][i]==false &&l!=-1&&r==-1)r = i-1;
		if(state[n-1][i]==true)water[i] = true;
	}
	if(l==-1)return ;
	if(r==-1)r = m-1;
	nodes[nodes_len++].l = l;
...
//	for(int i = 0;i<nodes_len;i++)std::cout<<nodes[i].l<<" "<<nodes[i].r<<std::endl;
//	std::cout<<std::endl;
