...
int main()
{
	...
	for (int i = 2; i <= tot; i++) {
		int limit;
		if (w % k == 0) limit = 0x3f3f3f3f;
		else if (i == tot) limit = (1 << (w % k)) - 1;
		else limit = 0x3f3f3f3f;

		for (int j = lim - i + 1; j > 0; j--) {
			dp[i][j] = dp[i][j + 1] + dp[i - 1][j + 1];
			if (j <= limit) ans += dp[i][j];
		}
	}
